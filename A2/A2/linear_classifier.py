...
def svm_loss_vectorized(
    ...
    - gradient of loss with respect to weights W; a tensor of same shape as W
    """
    # Ensure X and W have the same dtype
    ...
    margins = torch.clamp(margins, min=0) # 0 if margin is negative
...
def test_one_param_set(
    ...
    # num_val = data_dict['X_val'].shape[0]
    # w,loss_history = train_linear_classifier(svm_loss_vectorized, None, 
    #           data_dict['X_train'], 
    #           data_dict['y_train'], lr,
    #          reg, num_iters)
    # train_pred = predict_linear_classifier(w, data_dict['X_train'])
    # val_pred = predict_linear_classifier(w, data_dict['X_val'])
    ...
    train_acc = 100.0 * (data_dict['y_train'] == train_pred).double().mean().item()
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.max(dim=0, keepdim=True).values # avoid overflow
        correct_class_score = scores[y[i]]
        total=0
        for j in range(num_classes):
            total += torch.exp(scores[j])
        loss += -correct_class_score + torch.log(total)
        for j in range(num_classes):
           if j == y[i]:
               dW[:, j] += X[i] * (torch.exp(scores[j]) / total - 1)
           else:
               dW[:, j] += X[i] * (torch.exp(scores[j]) / total)
    loss /= num_train
    loss += reg * torch.sum(W * W)
    dW /= num_train
    dW += 2 * reg * W
...
def softmax_loss_vectorized(
    ...
    loss = -torch.sum(torch.log(probs[torch.arange(num_train), y])) / num_train + reg * torch.sum(W * W)
    identity_matrix = torch.eye(num_classes, device=probs.device)
    dW = X.T @ (probs - identity_matrix[y]) / num_train + 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
