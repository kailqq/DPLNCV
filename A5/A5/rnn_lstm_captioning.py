...
def rnn_step_backward(dnext_h, cache):
    ...
    # temp = x @ Wx + prev_h @ Wh + b, next_h = tanh(temp)
    dtemp = dnext_h * (1 - next_h ** 2) # shape:(N,H)
    dx = dtemp @ Wx.T # shape:(N,D)
    dprev_h = dtemp @ Wh.T # shape:(N,H)
    dWx = x.T @ dtemp # shape:(D,H)
    dWh = prev_h.T @ dtemp # shape:(H,H)
    db = dtemp.sum(0) # shape:(H,)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.image_encoder_layer = ImageEncoder(pretrained=image_encoder_pretrained) 

        if self.cell_type == "attn":
            self.conv = nn.Conv2d(self.image_encoder_layer.out_channels, hidden_dim, 1, 1)
        else:
            self.avg_pool_layer = nn.AvgPool2d(4, 4)
            self.transform_layer = nn.Linear(self.image_encoder_layer.out_channels, hidden_dim)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        H = prev_h.shape[1]
        temp = x.mm(self.Wx) + prev_h.mm(self.Wh) + self.b # (N, 4H)
        ai, af, ao, ag = torch.split(temp, [H, H, H, H], dim = 1)
        i = torch.sigmoid(ai)
        f = torch.sigmoid(af)
        o = torch.sigmoid(ao)
        g = torch.tanh(ag)
...
def dot_product_attention(prev_h, A):
    ...
    # print(flatten_A.transpose(1, 2))
    # print(prev_h.unsqueeze(dim=2))
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        H = prev_h.shape[1]
        temp = x.mm(self.Wx) + prev_h.mm(self.Wh) + attn.mm(self.Wattn) + self.b # (N, 4H)
        ai, af, ao, ag = torch.split(temp, [H, H, H, H], dim = 1)
        i = torch.sigmoid(ai)
        f = torch.sigmoid(af)
        o = torch.sigmoid(ao)
