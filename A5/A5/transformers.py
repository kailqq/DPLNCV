...
def scaled_dot_product_two_loop_single(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out=torch.zeros(query.shape[0], query.shape[1])
    K,M=query.shape
    for i in range(K):
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        for layer in self.multihead:
          ...
          out_y.append(temp_y)
        concat_y = torch.cat(out_y, dim = 2)
        #print(concat_y.shape,out_y[0].shape,len(out_y))
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.gamma = nn.Parameter(torch.ones(emb_dim))  # 缩放参数
        self.beta = nn.Parameter(torch.zeros(emb_dim))  # 偏移参数
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        mean = x.mean(dim=-1, keepdim=True)  # (N, K, 1)
        
        # 计算方差
        var = ((x - mean) ** 2).mean(dim=-1, keepdim=True)  # (N, K, 1)
        
        # 归一化
        x_norm = (x - mean) / torch.sqrt(var + self.epsilon)  # (N, K, M)
