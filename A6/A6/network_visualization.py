...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        scores = model(X_adv)
        correct_scores = scores[range(X_adv.shape[0]), target_y]
        loss = correct_scores.sum()
        ...
        X_adv.grad.data.zero_()
        scoresx=model(X)
        correct_scoresx=scoresx[range(X.shape[0]), target_y]
        if(i % 10 == 0):
            print(f"Iteration {i+1} loss: {loss.item()},target_y: {target_y},adv_score: {correct_scores.item()},origin_score: {correct_scoresx.item()}")
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return X_adv
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    ########################################################################
    # Replace "pass" statement with your code
    reg_loss = l2_reg * (img.norm() ** 2)
    scores = model(img)
    correct_scores = scores[range(img.shape[0]), target_y]
    loss = correct_scores.sum() + reg_loss
