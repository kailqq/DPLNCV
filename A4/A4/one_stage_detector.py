...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.cls_conv1=nn.Conv2d(in_channels, stem_channels[0], kernel_size=3, padding=1,stride=1)
        ...
        self.box_conv1=nn.Conv2d(in_channels, stem_channels[0], kernel_size=3, padding=1,stride=1)
        ...
        # Replace "pass" statement with your code
        self.pred_cls=nn.Conv2d(stem_channels[-1], num_classes, kernel_size=3, padding=1,stride=1)
        nn.init.normal_(self.pred_cls.weight.data, mean=0.0, std=0.01)
        nn.init.constant_(self.pred_cls.bias.data, 0)

        self.pred_box=nn.Conv2d(stem_channels[-1], 4, kernel_size=3, padding=1,stride=1)
        nn.init.normal_(self.pred_box.weight.data, mean=0.0, std=0.01)
        nn.init.constant_(self.pred_box.bias.data, 0)

        self.pred_ctr=nn.Conv2d(stem_channels[-1], 1, kernel_size=3, padding=1,stride=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        x3=self.stem_cls(feats_per_fpn_level['p4'])
        class_logits['p4']=self.pred_cls(x3).flatten(start_dim=2).transpose(1,2)

        x4=self.stem_box(feats_per_fpn_level['p4'])
        boxreg_deltas['p4']=self.pred_box(x4).flatten(start_dim=2).transpose(1,2)
        centerness_logits['p4']=self.pred_ctr(x4).flatten(start_dim=2).transpose(1,2)
        
        x5=self.stem_cls(feats_per_fpn_level['p5'])
        class_logits['p5']=self.pred_cls(x5).flatten(start_dim=2).transpose(1,2)

        x6=self.stem_box(feats_per_fpn_level['p5'])
        boxreg_deltas['p5']=self.pred_box(x6).flatten(start_dim=2).transpose(1,2)
        centerness_logits['p5']=self.pred_ctr(x6).flatten(start_dim=2).transpose(1,2)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    mask_for_background = (deltas[:, 0] == -1)
    mask_for_valid = ~mask_for_background
  
    centerness = torch.sqrt(
        (torch.minimum(deltas[:, 0], deltas[:, 2]) * 
         torch.minimum(deltas[:, 1], deltas[:, 3])) / 
         (torch.maximum(deltas[:, 0], deltas[:, 2]) * 
          torch.maximum(deltas[:, 1], deltas[:, 3])))
    centerness = centerness * (mask_for_valid) - 1 * (mask_for_background)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for matched_boxes_per_fpn_level in matched_gt_boxes:
            ...
            for level_name in matched_boxes_per_fpn_level.keys():       
                matched_gt_deltas_per_fpn_level[level_name] = fcos_get_deltas_from_locations(locations_per_fpn_level[level_name], 
                                                                                             matched_boxes_per_fpn_level[level_name], 
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_scores, index = torch.max(level_pred_scores, dim = 1)
            # Step 2:
            # Replace "pass" statement with your code
            mask = (level_pred_scores > test_score_thresh).nonzero()
            level_pred_classes = index[mask].flatten()
            level_pred_scores = level_pred_scores[mask].flatten()

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[mask].reshape(-1, 4), 
                                                              level_locations[mask].reshape(-1, 2), 
                                                              self.backbone.fpn_strides[level_name])
            # (K, 4)
