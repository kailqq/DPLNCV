...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            shift=torch.tensor([[-new_width/2, -new_height/2, new_width/2, new_height/2]], dtype=temp_locations.dtype, device=temp_locations.device).expand(locations.shape[0], 4)
            anchor_boxes.append(temp_locations + shift)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    area_a = ((boxes1[:, 2]-boxes1[:, 0]) *
              (boxes1[:, 3]-boxes1[:, 1])).unsqueeze(1).expand_as(intersect)  # [M, N]
    area_b = ((boxes2[:, 2]-boxes2[:, 0]) *
              (boxes2[:, 3]-boxes2[:, 1])).unsqueeze(0).expand_as(intersect)  # [M, N]
...
def rcnn_get_deltas_from_anchors(
    ...
    pred_x = (anchors[:, 0] + anchors[:, 2])/2
    gt_x = (temp_gt_boxes[:, 0] + temp_gt_boxes[:, 2])/2
    pred_y = (anchors[:, 1] + anchors[:, 3])/2
    gt_y = (temp_gt_boxes[:, 1] + temp_gt_boxes[:, 3])/2
    pred_w = anchors[:, 2] - anchors[:, 0]
    gt_w = temp_gt_boxes[:, 2] - temp_gt_boxes[:, 0]
    pred_h = anchors[:, 3] - anchors[:, 1]
    gt_h = temp_gt_boxes[:, 3] - temp_gt_boxes[:, 1]
    t_x = (gt_x - pred_x) / pred_w
    t_y = (gt_y - pred_y) / pred_h
    t_w = torch.log(gt_w / pred_w)
    t_h = torch.log(gt_h / pred_h)
...
def rcnn_apply_deltas_to_anchors(
    ...
    b_x = pred_x + pred_w * temp_deltas[:, 0]
    b_y = pred_y + pred_h * temp_deltas[:, 1]
    b_w = pred_w * torch.exp(temp_deltas[:, 2])
    b_h = pred_h * torch.exp(temp_deltas[:, 3])
    x1 = b_x - b_w/2
    y1 = b_y - b_h/2
    x2 = b_x + b_w/2
    y2 = b_y + b_h/2
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                # step 1: Transform anchors to proposal boxes and clamp to image boundaries
                ...
                # step 2: Sort proposals by objectness and keep top-k
...
class FasterRCNN(nn.Module):
    ...
    def inference(
        ...
        pred_scores, indices = torch.max(pred_scores, dim = 1)
